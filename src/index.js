...
const {
    configEditor,
    extractMetadata : extractMetadataFromState,
} = util;

const event = "plugin:components:UPDATE_COMPONENTS";

const componentHelper = (component, state = {}, props) => ({
    ...state,
    ...
    const addNode = (treePosition, currentState = false, fullState) => {
        ...
        if(existingStateIndex > -1) {
            ...
            treePosition[treePosition.length] = {
                children  : [],
                state     : stateSegment,
                component : componentMap.get(fullState).meta.component.component,
                props     : componentMap.get(fullState).meta.component.props,
            };
        } else if(nextPath) {
...
export default ({
    ...
    config : (config = {}) => {
        ...
        result = configEditor.addContext(result, { [context] : [] });
